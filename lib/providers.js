export async function fetchCrossref(doi) {
  ...
  return {
    ...
    pdfUrl: (m.link || []).find((link) => link["content-type"] === "application/pdf")?.URL || null,
...
export async function fetchPmcIds(pmcid) {
  ...
  const params = new URLSearchParams({
    ...
    format: "json",
    tool: "refdown",
    email: "adycovs@gmail.com"
  });
  const r = await fetch(`https://pmc.ncbi.nlm.nih.gov/tools/idconv/api/v1/articles/?${params}`);
  if (!r.ok) return null;
  const data = await r.json();
  const record = data.records?.[0];
  if (!record || record.status === "error") return null;
  return {
    pmcid: record.pmcid || id,
